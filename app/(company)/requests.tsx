        ...
        <View style={styles.requestInfo}>
          ...
          {__DEV__ && (
            <Text style={styles.debugText}>
              ID: {item.id || 'INVALID'} | Verificación: {permissionCheck.verificationMethod} | Rol: {userRole || 'N/A'}
            </Text>
          )}
          {/* Show warning for invalid ID */}
          {!hasValidId && (
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        {user && (
          <>
            <Text style={styles.debugText}>
              Usuario: {user.email} | Rol: {userRole || 'N/A'} | DEV: {__DEV__ ? 'SI' : 'NO'}
            </Text>
            <View style={styles.debugButtonsContainer}>
              <TouchableOpacity
                ...
                onPress={async () => {
                  try {
                    const { debugJoinRequests } = await import('@/debug-requests-cleanup');
                    const result = await debugJoinRequests(empresaId!);
                    Alert.alert(
                      'Debug Results',
                      ...
                      [
                        ...
                        {
                          text: 'Ver Consola',
                          onPress: () => console.log('Debug result:', result)
                        }
                      ]
                    );
                  } catch (error) {
              ...
              >
                <Text style={styles.debugButtonText}>Debug Requests</Text>
              </TouchableOpacity>
              
              <TouchableOpacity
                style={[styles.debugButton, styles.offlineButton]}
                onPress={async () => {
                  try {
                    const { OfflineModeDebugger } = await import('@/debug-offline-mode');
                    await OfflineModeDebugger.toggleOfflineMode();
                    
                    const isOffline = OfflineModeDebugger.isOffline();
                    Alert.alert(
                      'Modo Offline',
                      isOffline 
                        ? '🔴 Firestore está ahora OFFLINE - Las operaciones de red fallarán'
                        : '🟢 Firestore está ahora ONLINE - Las operaciones de red funcionarán',
                      [{ text: 'OK' }]
                    );
                  } catch (error) {
                    console.error('Offline toggle error:', error);
                    Alert.alert('Error', String(error));
                  }
                }}
              >
                <Text style={styles.debugButtonText}>Toggle Offline</Text>
              </TouchableOpacity>
            </View>
          </>
        )}
      ...
      {/* Floating offline debug button - always visible */}
      <TouchableOpacity
        style={styles.floatingOfflineButton}
        onPress={async () => {
          try {
            const { OfflineModeDebugger } = await import('@/debug-offline-mode');
            await OfflineModeDebugger.toggleOfflineMode();
            ...
            Alert.alert(
              'Modo Offline Debug',
              isOffline 
                ? '🔴 Firestore OFFLINE - Las operaciones de red fallarán'
                : '🟢 Firestore ONLINE - Las operaciones de red funcionarán',
              [{ text: 'OK' }]
            );
          } catch (error) {
            console.error('Offline toggle error:', error);
