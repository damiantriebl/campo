...
const ClientModule = ({ 
    ...
    const formatDebt = (debt: number) => {
        ...
        if (debt > 0) return `Debe $${debt.toFixed(2)}`;
        return `A favor $${Math.abs(debt).toFixed(2)}`;
    };

    const getDebtColor = (debt: number) => {
        if (debt === 0) return colors.success; // Green for no debt
            ...
            <View style={[styles.cardHeader, { marginBottom: spacing.xs }]}>
                <View style={styles.clientInfo}>
                    ...
                    <Text style={[
                        styles.debtText, 
                        { 
                            color: getDebtColor(client.deudaActual),
                            fontSize: typography.fontSize.md,
                            fontWeight: typography.fontWeight.semibold,
                        }
                    ]}>
                        {formatDebt(client.deudaActual)}
                    </Text>
                </View>
